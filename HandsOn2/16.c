...
int main(){
    ...
    if(f == -1){
        ...
        if(read(p2[0],&b,sizeof(int)) == -1){
            perror("Error Writing");
            exit(0);
        }
        printf("Value of b read in parent process is %d\n",b);
        close(p2[0]);
        close(p1[1]);
    }else{
        sleep(1);
        ...
        printf("\nEnter b:");
        scanf("%d",&b);
        if(write(p2[1],&b,sizeof(int)) == -1){
            perror("Error Writing");
            exit(0);
        }   
        printf("Value of b written in child process is %d\n",b);
...
Enter b:6
Value of b written in child process is 6
Value of a read in child process is 4
Value of b read in parent process is 6
============================================================================
*/